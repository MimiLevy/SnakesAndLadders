namespace tsgame {
    ...
    const btnDice: HTMLButtonElement = document.querySelector("#btnDice");
    const btnStep: HTMLButtonElement = document.querySelector("#btnStep");
    const allSqures: Element[] = [];
    const snakeSpots: Element[] = [];
    const ladderSpots: Element[] = [];
    ...
    allSqures.sort((a: any, b: any) => a.id - b.id);
    ...
    function checkWinner(nextIndex: number) {
        if (nextIndex === 99) {
            winnerSound.play();
            let winner = true;
            showMessage(winner);
    ...
    function checkIt(snakeOrLadder: string, alertContent: string) {
    ...
    function showMessage(winner: boolean) {
